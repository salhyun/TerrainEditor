...
{
public:
	enum INTERSECT {INTERSECT_NONE=0, INTERSECT_TRIANGLE, INTERSECT_CYLINDER, INTERSECT_SPHERE} ;
  ...
  */
  ...
  virtual void getCollidingTriangles(float *pVertices, bool ModelSpace) = 0;
  ...
  */
  ...
  virtual bool getCollidingPlaneNormal(float nor[3], bool ModelSpace) = 0;

  virtual int projectileSphereCollision(float origin[3], float direction[3], float radius, bool closest, float segmin, float segmax) = 0;
  virtual int intersectBoundingBox(float origin[3], float direction[3], float radius, float segmin, float segmax) = 0;
};
